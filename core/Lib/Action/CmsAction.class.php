	...
	* @返回赋值后的style数组,可在调用的类直接assign*/
    public function tags_style(){
		...
		if(C('url_html')){
			...
			$style['movieurl'] = C('web_path').'movie'.C('html_file_suffix');
			$style['tvurl'] = C('web_path').'tv'.C('html_file_suffix');
			$style['zongyiurl'] = C('web_path').'zongyi'.C('html_file_suffix');
			$style['animeurl'] = C('web_path').'anime'.C('html_file_suffix');
			$style['jilupianurl'] = C('web_path').'jilupian'.C('html_file_suffix');
			...
			$style['movieurl'] = get_show_url('movie','',1);
			$style['tvurl'] = get_show_url('tv','',1);
			$style['zongyiurl'] = get_show_url('zongyi','',1);
			$style['animeurl'] = get_show_url('anime','',1);
			$style['jilupianurl'] = get_show_url('jilupian','',1);
	...
	* @返回赋值后的arrays 多维数组*/
	public function tags_video_read($array,$playarr=false){
		...
		if($playarr){
			...
			$currentUrl=$this->getCurrentVedioUrl($array['playurl'],$videoji);
			...
			if (C('user_pay')  && in_array($array['cid'],C('user_paycid'))){
				$player .='<div id="GxInstall"></div><div id="GxPlayer" class="Userpay"></div>';
				 ...
				 if(stripos($currentUrl, 'bdhd://')!==false){//百度影音
				...
				}else if(stripos($currentUrl, 'qvod://')!==false){//快播
					$player .='<div id="GxInstall"></div><div id="GxPlayer" class="Loading"></div>';
					$player .='<script language="javascript" type="text/javascript">'."\n";
					$player .='var $playlist="'.str_replace(array("\r\n", "\n", "\r"),'+++',$array['playurl']).'"'."\n";
					$player .='</script>'."\n";
					$player .='<script language="javascript" src="'.C('web_path').'views/js/qvod.js" charset="utf-8"></script>';
				...
				}else if(stripos($currentUrl, 'xfplay://')!==false){//xf影音
					$player .='<div id="GxInstall"></div><div id="GxPlayer" class="Loading"></div>';
					$player .='<script language="javascript" type="text/javascript">'."\n";
					$player .='var $playlist="'.str_replace(array("\r\n", "\n", "\r"),'+++',$array['playurl']).'"'."\n";
					$player .='</script>'."\n";
					$player .='<script language="javascript" src="'.C('web_path').'views/js/xfplayer.js" charset="utf-8"></script>';
				}else if(stripos($currentUrl, 'niba://')!==false){  //泥巴影音
					$player .='<div id="GxInstall"></div><div id="GxPlayer" class="Loading"></div>';
					$player .='<script language="javascript" type="text/javascript">'."\n";
					$player .='var $playlist="'.str_replace(array("\r\n", "\n", "\r"),'+++',$array['playurl']).'"'."\n";
					$player .='</script>'."\n";
					$player .='<script language="javascript" src="'.C('web_path').'views/js/niba.js" charset="utf-8"></script>';
				}else if(stripos($currentUrl, 'ffhd://')!==false){  //非凡影音
					$player .='<div id="GxInstall"></div><div id="GxPlayer" class="Loading"></div>';
					$player .='<script language="javascript" type="text/javascript">'."\n";
					$player .='var $playlist="'.str_replace(array("\r\n", "\n", "\r"),'+++',$array['playurl']).'"'."\n";
					$player .='</script>'."\n";
					$player .='<script language="javascript" src="'.C('web_path').'views/js/ffhd.js" charset="utf-8"></script>';
				}else if(stripos($currentUrl, 'ftp://')!==false){  //西瓜影音
					$currentUrl = str_replace('ftp://', 'xg://', $currentUrl);
					$player .='<div id="GxInstall"></div><div id="GxPlayer" class="Loading"></div>';
					$player .='<script language="javascript" type="text/javascript">'."\n";
					$player .='var $playlist="'.str_replace(array("\r\n", "\n", "\r"),'+++',$array['playurl']).'"'."\n";
					$player .='</script>'."\n";
					$player .='<script language="javascript" src="'.C('web_path').'views/js/xigua.js" charset="utf-8"></script>';
				}else {
					$player ="未找到合适的播放器！";
				}
			}
			...
			$array['webtitle']  = '正在播放_'.$array['title'].'_'.$array['playname'].'_'.C('web_name').'_'.C('web_url');
			$array['player']    = $player;
			//显示点击数
			...
			if($channel[0]['pid']){
			$array['webtitle']  = $array['title'].'在线播放'.'_'.$channel[0]['cname'].'_'.$channel[0]['cname_p'].'_'.C('web_name').'_'.C('web_url');
			}	
			else{
					$array['webtitle']  = $array['title'].'在线播放'.'_'.$channel[0]['cname'].'_'.C('web_name').'_'.C('web_url');
			}
	 ...
	 * 获取当前播放地址与影片名称
	 */
	private  function  getCurrentVedioUrl($arrayUrl,$videoji){
		try{
			if($videoji<0){
				$videoji=0;
			}
			$playurls =str_replace(array("\r\n", "\n", "\r"),'+++',$arrayUrl);
			$current =split("\+\+\+", $playurls);
			return trim($current[$videoji]);
		}catch(Exception $e){
			print  $e->getMessage();
		}
	}
